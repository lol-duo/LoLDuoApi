...
public interface SoloMatchRepository extends JpaRepository<SoloMatchEntity, Long> {
    ...
    @Query(value = "select * from solo_match where all_count > ?3 and  position like ?1 and champion_id like ?2 order by win_count / all_count DESC limit 30",nativeQuery = true)
