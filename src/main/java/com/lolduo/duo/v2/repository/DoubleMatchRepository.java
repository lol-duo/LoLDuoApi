...
public interface DoubleMatchRepository extends JpaRepository<DoubleMatchEntity, Long> {
    ...
    @Query(value = "select * from double_match where all_count > ?5 and  position1 like ?1 and champion_id1 like ?2 and position2 like ?3 and champion_id2 like ?4 order by win_count / all_count DESC limit 30",nativeQuery = true)
