...
public interface TripleCombiRepository extends JpaRepository<TripleCombiEntity,Long>, ICombiRepository {
    ...
    List<TripleCombiEntity> findAllByChampionIdAndPositionGroupByPositionWinRateDesc(String championId, String position, String positionList, String excludePositionList, double minPickRate);

    @Query(value = "select id, position, champion_id, item_list, perk_list, spell_list, perk_myth_item, win_count_sum as win_count, all_count_sum as all_count from (select *, sum(win_count) as win_count_sum, sum(all_count) as all_count_sum from triple_combi group by position having all_count_sum >= (select count(*)/2 from penta_match) * ?5) t1 where json_contains(champion_id,?1) and json_contains(position,?2) and json_contains(json_extract(position, '$.*'), ?3) and not json_has_exclude_position(position, ?1, ?4) order by win_count_sum / all_count_sum ASC limit 100",nativeQuery = true)
    List<TripleCombiEntity> findAllByChampionIdAndPositionGroupByPositionWinRateAsc(String championId, String position, String positionList, String excludePositionList, double minPickRate);

    @Query(value = "select id, position, champion_id, item_list, perk_list, spell_list, perk_myth_item, win_count_sum as win_count, all_count_sum as all_count from (select *, sum(win_count) as win_count_sum, sum(all_count) as all_count_sum from triple_combi group by position having all_count_sum >= (select count(*)/2 from penta_match) * ?5) t1 where json_contains(champion_id,?1) and json_contains(position,?2) and json_contains(json_extract(position, '$.*'), ?3) and not json_has_exclude_position(position, ?1, ?4) order by all_count_sum DESC limit 100",nativeQuery = true)
    List<TripleCombiEntity> findAllByChampionIdAndPositionGroupByPositionGameCountDesc(String championId, String position, String positionList, String excludePositionList, double minPickRate);

    @Query(value = "select id, position, champion_id, item_list, perk_list, spell_list, perk_myth_item, win_count_sum as win_count, all_count_sum as all_count from (select *, sum(win_count) as win_count_sum, sum(all_count) as all_count_sum from triple_combi group by position having all_count_sum >= (select count(*)/2 from penta_match) * ?5) t1 where json_contains(champion_id,?1) and json_contains(position,?2) and json_contains(json_extract(position, '$.*'), ?3) and not json_has_exclude_position(position, ?1, ?4) order by all_count_sum ASC limit 100",nativeQuery = true)
    List<TripleCombiEntity> findAllByChampionIdAndPositionGroupByPositionGameCountAsc(String championId, String position, String positionList, String excludePositionList, double minPickRate);
