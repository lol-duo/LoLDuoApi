...
public interface DoubleCombiRepository extends JpaRepository<DoubleCombiEntity,Long>, ICombiRepository {
    ...
    @Query(value = "select * from double_combi where all_count >= ?2 and json_contains(position,?1) and perk_myth_item not like '%|0%' order by win_count / all_count DESC limit 1",nativeQuery = true)
