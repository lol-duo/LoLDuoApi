...
import com.lolduo.duo.repository.clientInfo.combiSummaryRepository.*;
...
public class ClientService {
    ...
    private final SoloCombiSummaryRepository soloCombiSummaryRepository;
    private final DoubleCombiSummaryRepository doubleCombiSummaryRepository;
    private final TripleCombiSummaryRepository tripleCombiSummaryRepository;
    private final PentaCombiSummaryRepository pentaCombiSummaryRepository;
    ...
    private ICombiSummaryRepository getCombiSummaryRepository(int championCount) {
        if (championCount == 1) {
            log.info("getCombiSummaryRepository() - championCount : {}, 1명", championCount);
            return soloCombiSummaryRepository;
        }
        else if (championCount == 2) {
            log.info("getCombiSummaryRepository() - championCount : {}, 2명", championCount);
            return doubleCombiSummaryRepository;
        }
        else if (championCount == 3) {
            log.info("getCombiSummaryRepository() - championCount : {}, 3명", championCount);
            return tripleCombiSummaryRepository;
        }
        else if (championCount == 5) {
            log.info("getCombiSummaryRepository() - championCount : {}, 5명", championCount);
            return pentaCombiSummaryRepository;
        }
        else {
            log.info("getCombiSummaryRepository() - 요청 챔피언 개수가 올바르지 않습니다. (1, 2, 3, 5명만 가능)");
            return null;
        }
    }
    ...
    private void putCombiInfoToResult(List<ChampionInfoList> result, List<? extends ICombiSummaryEntity> combiSummaryEntityList, List<ChampionInfoDTO> championInfoDTOList, Map<String, Long> selectedPositionOrderMap, Map<Long, Long> selectedChampionOrderMap, Queue<ChampionInfoResponse> allChampAllPositionQueue) {
        if (combiSummaryEntityList != null && !combiSummaryEntityList.isEmpty()) {
            log.info("putCombiInfoToResult() - 조합 정보.");
            combiSummaryEntityList.forEach(combiSummaryEntity -> {
                log.info("putCombiInfoToResult() - 검색 결과 : championId = {}, position = {}, AllCountSum = {}, WinCountSum = {}, WinRate = {}", combiSummaryEntity.getChampionId().toString(), combiSummaryEntity.getPosition().toString(), combiSummaryEntity.getAllCountSum(), combiSummaryEntity.getWinCountSum(), combiSummaryEntity.getWinRate());
                result.add(new ChampionInfoList(
                                createChampionInfoResponseList(combiSummaryEntity.getChampionId().size(), combiSummaryEntity, selectedPositionOrderMap, selectedChampionOrderMap, allChampAllPositionQueue),
                                String.format("%.2f%%", 100 * combiSummaryEntity.getWinRate()),
                        String.valueOf(combiSummaryEntity.getAllCountSum()).replaceAll("\\B(?=(\\d{3})+(?!\\d))", ",") + " 게임"
                        )
                );
            });
        }
    ...
    private List<ChampionInfoResponse> createChampionInfoResponseList(int championCount, ICombiSummaryEntity combiSummaryEntity, Map<String, Long> selectedPositionOrderMap, Map<Long, Long> selectedChampionOrderMap, Queue<ChampionInfoResponse> allChampAllPositionQueue) {
        ChampionInfoResponse[] championInfoResponseArray = new ChampionInfoResponse[championCount];

        for (Map.Entry<Long, String> positionEntry : combiSummaryEntity.getPosition().entrySet()) {
            Long order = -1L;
